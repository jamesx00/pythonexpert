...
with patch('builtins.print') as patched_print:
    ...
    try:
        ...
        patched_print.assert_called_with("You cannot divide a number with a string!")
        results[4] = True
    except Exception as e:
        results[4] = False
    ...
    try:
        ...
        assert re.search('except.*?ZeroDivisionError.*?:', file_content) is None
        assert re.search('except.*?TypeError.*?:', file_content) is None
        results[6] = True
    except Exception as e:
        results[6] = False
