...
try:
    ...
    cur.execute("SELECT * FROM products WHERE category IN ('Electronics', 'Fitness');")
    expected_columns = [c[0] for c in cur.description]
    expected_table = cur.fetchall()

    results = {}

    category_values = get_column(result_table, result_columns, "category")
    results[1] = category_values is not None
