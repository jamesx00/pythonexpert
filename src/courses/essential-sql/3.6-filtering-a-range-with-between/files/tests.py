...
try:
    ...
    cur.execute("SELECT * FROM products WHERE price BETWEEN 20 AND 50;")
    expected_columns = [c[0] for c in cur.description]
    expected_table = cur.fetchall()

    results = {}

    price_values = get_column(result_table, result_columns, "price")
    results[1] = price_values is not None
