...
try:
    ...
    cur.execute("SELECT * FROM orders WHERE shipped_date IS NULL;")
    expected_columns = [c[0] for c in cur.description]
    expected_table = cur.fetchall()

    results = {}

    shipped_date_values = get_column(result_table, result_columns, "shipped_date")
    results[1] = shipped_date_values is not None
