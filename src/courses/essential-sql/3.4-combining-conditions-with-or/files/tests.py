...
try:
    ...
    cur.execute("SELECT * FROM orders WHERE status = 'Pending' OR status = 'Cancelled';")
    expected_columns = [c[0] for c in cur.description]
    expected_table = cur.fetchall()

    results = {}

    status_values = get_column(result_table, result_columns, "status")
    results[1] = status_values is not None
